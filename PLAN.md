...
*As built*:
- **Error-code map: already complete.** All 29 codes in `API.md` are present in both `ApiErrorCode` and `ERROR_COPY` — verified by diffing the codes extracted from `API.md` against the union, not by reading. `ERROR_COPY` is typed `Record<ApiErrorCode, …>`, so the compiler keeps it complete from here.
- **Breadcrumb home pointed at `/`** — the public marketing page. Inside the authenticated shell that is not what a home icon promises; it now points at `/dashboard`. `BreadcrumbService` was also rewritten: the old template version looped over `route.children` but returned inside the loop, so it only ever walked the first child.
- **Detail pages had no trail.** `company/rentals/:rentalId` is a flat route, not a child of `company/rentals`, so the router tree alone cannot produce `Rental requests / Request`. Rather than restructure routes for a widget, a detail route names its list with `data.breadcrumbParent`. The public layout renders no breadcrumb, so public routes carry `breadcrumb` for the page title only.
- **One dead session produced N toasts.** A failed refresh is delivered to every request that was waiting on it, and each ran the teardown. `AuthService.endExpiredSession()` now reports whether *this* caller ended the session; only that one toasts and redirects.
- **The car-detail booking panel rendered a bare number** (`dailyPriceFrom.toFixed(2)`) with no currency symbol — the only money in the app that bypassed `formatMoney`. Fixed. A scan confirms every other price and date goes through `shared/utils/format.ts`.
- **Accessibility**: the account menu was a `pStyleClass` class toggle, which no component could observe, so `aria-expanded` had no honest value — it is now a signal-driven disclosure with outside-click and Escape. Menu and theme-toggle buttons gained names, decorative icons in the topbar are `aria-hidden`, the landing datepickers gained `ariaLabel` (their "labels" were `<span>`s), the global progress bar is now `aria-hidden` (each page announces its own loading), and the five list pages carry an `sr-only role="status"` region that announces loading and then the result count. Detail pages were left out on purpose: they load once per navigation, which the title change already announces.
- **Testimonials: deleted, not replaced.** No reviews feature exists and there are no real testimonials to substitute, so the band, the "4.8 ★ · 12k reviews" badge, `PLACEHOLDER_REVIEWS` and the now-dangling "Reviews" nav link are gone. Restoring them from git history would restore fabricated content — write real ones instead.
- **`PlaceholderPage` and `soon()` deleted** in Phase 7; every route is real.
- **Not done — the two items that need a browser.** The design audit (light/dark at 360/768/1280, contrast of lime-on-green and muted-on-cream) and the end-to-end persona smoke including raced-conflict paths were not performed: no browser and no running backend in this environment. Everything above is code-level. The contrast pairs in particular are a real risk and have never been measured.
...
**Content debt — resolved in Phase 8.** The landing page's "Trusted by drivers" band (three testimonials and a "4.8 ★ · 12k reviews" badge) was placeholder copy carried over from the design file. There is no reviews feature, so none of it was real and none of it could be made real; the section, the badge, the `PLACEHOLDER_REVIEWS` constant and the "Reviews" nav link were **deleted**. If genuine, attributable testimonials ever exist they should be written fresh — reviving the old block from git history would revive fabricated content. The same bar applies to any product claim in the marketing copy: if it promises a behaviour, there must be an endpoint behind it.
